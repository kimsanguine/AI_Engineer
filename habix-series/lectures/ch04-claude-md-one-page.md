...
<!-- SCREENSHOT v1.1
  위치: ch04-claude-md-one-page.md:36
  before: "프로젝트 설명: 음식 배달 앱" 한 줄만 있는 CLAUDE.md
  after: 8개 섹션이 모두 채워진 CLAUDE.md
-->
| | Before | After |
|---|---|---|
| **스크린샷** | *"프로젝트 설명: 음식 배달 앱" 한 줄만 있는 CLAUDE.md* *(v1.1 예정)* | *8개 섹션이 모두 채워진 CLAUDE.md* *(v1.1 예정)* |
...
-->
> **[그림]** *전역(~/.claude/) → 저장소 루트 → 하위 폴더 순서로 쌓이는 CLAUDE.md 3-Layer 구조를 수직 단면도로 표현* *(v1.1에서 추가 예정)*
...
- **600줄까지 부풀린다.** 길수록 좋은 게 아닙니다. 긴 문서일수록 모델이 중간을 흘려 읽습니다(Lost in the middle). 50~150줄을 마지노선으로 잡으세요. Eugene Yan은 이렇게 부풀린 `CLAUDE.md`를 *"문맥세(context tax)"* 라고 부릅니다. 세션을 열 때마다 안 쓰는 정보까지 매번 다 읽혀서, 정작 필요한 토큰 예산이 줄어든다는 뜻입니다.
